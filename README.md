...
这个项目适于小型实验室、学生项目、研发样机和内部器件库管理。你可以直接描述板子的目标，例如“我要做一个三音频谱测试板”或“需要一个 16 bit DAC，再用 ADC 采集输出”，系统会结合当前库存给出候选器件，并在你确认后记录到项目方案中。

## 功能

通过对话辅助选型：用自然语言描述需求，Agent 给出简短的工程建议和候选器件，不同项目拥有独立的聊天记录、推荐结果和已采用器件。
器件库管理：维护型号、类别、封装、库存数量、库位和关键参数。用户确认采用器件后，可以继续做预占、补货和出库记录。
数据手册导入：上传 PDF 后自动提取器件信息，先预览和编辑，再确认入库。
可视化界面：提供 Agent 对话页和芯片库管理页，适合本地部署使用。

## 技术栈
FastAPI/SQLAlchemy/LangGraph/OpenAI-compatible SDK/SQLite/原生 HTML / CSS / JavaScript
